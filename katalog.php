...
<body class="bg-purple-50">
    <nav class="bg-white shadow-md sticky top-0 z-20">
        <div class="container mx-auto px-6 py-3 flex justify-between items-center">
            <a href="katalog.php" class="text-xl font-bold text-purple-800">SIMPRAK</a>
            <div class="flex items-center space-x-4">
                <?php if (isset($_SESSION['user_id'])): ?>
                    <a href="<?php echo $_SESSION['role']; ?>/dashboard.php" class="text-gray-600 hover:text-purple-700">Dashboard Saya</a>
                    <a href="logout.php" class="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-md text-sm font-medium transition-colors">Logout</a>
                <?php else: ?>
                    <a href="login.php" class="text-gray-600 hover:text-purple-700">Login</a>
                    <a href="register.php" class="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-md text-sm font-medium transition-colors">Register</a>
                <?php endif; ?>
            </div>
        </div>
    </nav>
    ...
    <div class="container mx-auto p-6">
        <div class="text-center bg-white p-8 rounded-xl shadow-lg mb-8">
            <h1 class="text-4xl font-extrabold text-purple-900">Katalog Mata Praktikum</h1>
            ...
            <form method="GET" action="katalog.php" class="mt-6 max-w-lg mx-auto">
                <div class="flex">
                    <input type="text" name="search" class="w-full px-4 py-3 border border-gray-300 rounded-l-md focus:outline-none focus:ring-2 focus:ring-purple-500" placeholder="Cari berdasarkan nama praktikum..." value="<?php echo htmlspecialchars($search_query); ?>">
                    <button type="submit" class="bg-purple-600 hover:bg-purple-700 text-white font-bold py-3 px-5 rounded-r-md">Cari</button>
                </div>
            </form>
        </div>
            ...
            <?php if ($all_practicums_result->num_rows > 0): ?>
                <?php while ($praktikum = $all_practicums_result->fetch_assoc()): ?>
                    <div class="bg-white rounded-xl shadow-md p-6 flex flex-col justify-between transform hover:-translate-y-2 transition-transform duration-300">
                        <div>
                            <h3 class="text-xl font-bold text-purple-900 mb-2"><?php echo htmlspecialchars($praktikum['nama_praktikum']); ?></h3>
                            ...
                            <?php if (isset($_SESSION['user_id']) && $_SESSION['role'] === 'mahasiswa'): ?>
                                ...
                                <?php else: ?>
                                    <button type="button"
                                        ...
                                        class="w-full bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded-lg transition-colors duration-300">
                                        Daftar Sekarang
                                    </button>
                                    <div id="modal-<?php echo $praktikum['id']; ?>" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 hidden">
                                        <div class="bg-white rounded-lg p-6 shadow-lg w-full max-w-md">
                                            <h3 class="text-lg font-bold mb-4 text-purple-700">Konfirmasi Pendaftaran</h3>
                                            ...
                                            <form id="form-daftar-<?php echo $praktikum['id']; ?>" onsubmit="return daftarPraktikumAjax(event, '<?php echo $praktikum['id']; ?>');">
                                                ...
                                                <div class="flex justify-end gap-2">
                                                    ...
                                                    <button type="submit" class="px-4 py-2 rounded bg-purple-600 hover:bg-purple-700 text-white font-bold">Iya</button>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
        ...
        function daftarPraktikumAjax(e, id) {
            ...
            .then(data => {
                ...
                if (data.status === 'success') {
                    alertBox.className = "mb-4 p-4 rounded-md text-center bg-green-100 text-green-800 transition-opacity duration-500";
                    ...
                    alertBox.className = "mb-4 p-4 rounded-md text-center bg-yellow-100 text-yellow-800 transition-opacity duration-500";
                    alertBox.innerText = "Kelas yang Anda pilih sudah penuh.";
                } else if (data.status === 'error') {
                    alertBox.className = "mb-4 p-4 rounded-md text-center bg-red-100 text-red-800 transition-opacity duration-500";
                    ...
                    alertBox.className = "mb-4 p-4 rounded-md text-center bg-red-100 text-red-800 transition-opacity duration-500";
                    alertBox.innerText = data.message || "Terjadi kesalahan yang tidak diketahui.";
                }
                setTimeout(() => {
            ...
            .catch(() => {
                ...
                const alertBox = document.getElementById('ajax-alert');
                alertBox.className = "mb-4 p-4 rounded-md text-center bg-red-100 text-red-800 transition-opacity duration-500";
                alertBox.innerText = "Terjadi masalah koneksi. Coba lagi nanti.";
                 setTimeout(() => {
                    alertBox.classList.add('opacity-0');
                    setTimeout(() => { alertBox.classList.add('hidden'); }, 500);
                }, 4000);
